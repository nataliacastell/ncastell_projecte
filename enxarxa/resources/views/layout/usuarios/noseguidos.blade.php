            ...
            <div class="mb-4">
                ...
                <form action="{{ route('follows.create') }}" method="POST">
                    ...
                    <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded"
                        id="followButton{{ $publicacion['usuario']['id'] }}">
                        {{ $isFollowing ? 'Dejar de seguir' : 'Seguir' }}
                    </button>
                </form>
            </div>
            <div>
                <p>{{ $publicacion['publicacion']['texto'] }}</p>
            </div>
            @if ($publicacion['publicacion']['imagen'])
            <div>
                <img src="{{ $publicacion['publicacion']['imagen'] }}" alt="{{ $publicacion['publicacion']['altText'] }}"
                    class="w-full h-auto rounded">
            </div>
            @endif
            <div class="mt-4">
                <p>Likes: {{ $publicacion['publicacion']['likes'] }}</p>
            </div>
        </div>
        @endforeach
    </div>
